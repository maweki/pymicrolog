...
class Program(object):
  ...
  def __init__(self, rules, name=None, fnmapping=dict(), reorderBodies=True):
      ...
      if isinstance(rule.head, TempAnnotatedFormula):
        if rule.head.temporalAnnotation is start:
          if rule.body is not None:
            raise ValueError("Rule Body for start facts must be empty")
        ...
        if rule.body is None:
          ...
          self.always.append(rule)
        elif isinstance(rule.body, Conjunction) and all(isinstance(formula, OracleFormula) or isinstance(formula, NegatedOracleFormula) for formula in rule.body.asConjunction()):
            self.always.append(rule)
        else:
    ...
    assert numberOfRules == len(self.initial) + len(self.next) + len(self.always) + len(unstratified)
    ...
    for rule in unstratified:
        ...
        if isinstance(rule.body, Formula) or isinstance(rule.body, NegatedFormula):
